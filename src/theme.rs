//! Motyw kolorystyczny — domyślnie "tron" z eDEX-UI.
//! Można wczytać dowolny plik motywu eDEX-UI przez zmienną NGTERM_THEME=/sciezka/do/tron.json
...
impl Color {
    ...
    /// Ta sama barwa z inną alfą.
    pub fn alpha(self, a: f32) -> Self {
        Color { a, ..self }
    }

    /// Interpolacja w stronę czerni (przygaszenie).
...
pub struct Theme {
    /// Główny kolor akcentu (u eDEX: rgb(r,g,b) z motywu).
    pub base: Color,
    /// Tło całego UI (light_black).
    pub bg: Color,
    /// Kolor siatki/szarości (grey).
    pub grey: Color,
    pub term_fg: Color,
    pub term_bg: Color,
    pub cursor: Color,
    /// Paleta ANSI 16 kolorów dla terminala.
    pub ansi: [Color; 16],
}

impl Theme {
    /// Motyw "tron" — domyślny motyw eDEX-UI.
    pub fn tron() -> Self {
    ...
    /// Wczytuje motyw w formacie eDEX-UI (themes/*.json).
    pub fn from_edex_json(text: &str) -> Option<Self> {
        ...
        // Niektóre motywy eDEX definiują pełną paletę w terminal.colors
    ...
    pub fn load() -> Self {
        if let Ok(path) = std::env::var("NGTERM_THEME") {
            if let Ok(text) = std::fs::read_to_string(&path) {
                ...
                eprintln!("ng-term: nie udało się sparsować motywu {path}, używam 'tron'");
            }
        }
        Self::tron()
    }
...
/// Kolor z indeksu 256-kolorowej palety xterm.

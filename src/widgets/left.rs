//! Lewa kolumna: zegar, informacje o systemie, inspektor sprzętu,
//! wykresy CPU, pamięć (siatka kropek), lista procesów.
...
pub fn draw(ctx: &mut Ctx, col: Rect, snap: &Snapshot) {
    // Nagłówek kolumny jak w eDEX ("PANEL" + zegar systemowy w tytule kolumny).
    ...
    // Podział kolumny na moduły (ułamki wysokości kolumny).
    let gap = ctx.vh(1.2);
...
fn clock(ctx: &mut Ctx, r: Rect, _snap: &Snapshot) {
    ...
    // Osobne rysowanie segmentów, dwukropki migają jak w eDEX.
...
fn cpu(ctx: &mut Ctx, r: Rect, snap: &Snapshot) {
    ...
    // Wykresy per rdzeń w dwóch kolumnach jak w eDEX.
    ...
    for i in 0..n {
        ...
        // Poziomy pasek: wypełnienie ramki od lewej wg bieżącego użycia rdzenia.
        ...
        // Bieżąca wartość po prawej; gdy pasek dochodzi do tekstu,
        // rysujemy go kolorem tła dla czytelności.
    ...
    // Stopka: load average + temperatura.
...
fn memory(ctx: &mut Ctx, r: Rect, snap: &Snapshot) {
    ...
    // Siatka kropek jak w eDEX ramwatcher.
    let px = ctx.font_px(0.9);
    let text_h = px * 1.5;
    let swap_h = px * 1.5;
...
fn toplist(ctx: &mut Ctx, r: Rect, snap: &Snapshot) {
    ...
    // Geometria kolumn wspólna dla nagłówka i wierszy (font mono).
    let mono1 = ctx.fonts.measure(FONT_MONO, px, "0", 0.0);
    let pid_right = r.x + 6.0 * mono1;
    let name_x = r.x + px * 4.5;
    // Prawy blok: "{:>5.1}%" (CPU, 6 zn.) + spacja + "{:>4.1}%" (MEM, 5 zn.).
    let cpu_right = r.right() - 6.0 * mono1;

    // Nagłówki kolumn w miejscu dawnego tytułu — wartości trafiają
    // dokładnie pod nazwy kolumn.
    ...
    // Podkreślenie z "wąsami" jak w pozostałych modułach.

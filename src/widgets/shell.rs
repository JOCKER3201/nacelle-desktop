//! Centralny panel terminala — ramka ze ściętymi rogami (augmented-ui),
//! pasek zakładek ze skośnymi bokami jak w eDEX, siatka znaków.
...
/// Prostokąty zakładek — wspólna geometria dla rysowania i hit-testów w main.
...
/// Zwraca pożądany rozmiar siatki (kolumny, wiersze) — main dopasowuje PTY.
pub fn draw(
    ctx: &mut Ctx,
    r: Rect,
    term: &Term,
    ...
    // Ramka ze ściętymi narożnikami.
    ctx.dl
        .chamfer_frame(r.x, r.y, r.w, r.h, ctx.vh(1.1), ctx.vh(0.18).max(1.5), base.alpha(0.5));

    // Pasek zakładek (5 sztuk, skośne boki — skewX(35deg) jak w eDEX).
    ...
    // Rysowanie komórek (tylko tyle, ile faktycznie ma terminal).
    ...
    for y in 0..draw_rows {
        ...
        for x in 0..draw_cols {
            // Wiersze ze scrollbacku mogą mieć inną długość po zmianie rozmiaru.
    ...
    if term.cursor_visible && term.view_offset == 0 && term.cur_y < draw_rows {
        ...
        if blink {
            ...
            // Znak pod kursorem w kolorze tła.
    ...
    // Wskaźnik przewinięcia scrollbacku.
...
fn draw_cell_char(ctx: &mut Ctx, ch: char, x: f32, y: f32, ascent: f32, px: f32, color: Color) {
    // Rysujemy pojedynczy glif fontem monospace na linii bazowej komórki.
    let _ = ascent;
    let s = ch.to_string();
    ctx.dl.text(ctx.fonts, FONT_MONO, px, x, y, &s, color, 0.0);
}

/// Kolory komórki z uwzględnieniem SGR (bold/dim/inverse).

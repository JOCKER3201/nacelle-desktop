//! Panel sterowania programem (lewy dolny róg): EXIT i SETTINGS.
//! Przyciski wyglądają i zachowują się jak zakładki terminala:
//! skośne boki, podświetlenie po najechaniu, błysk po kliknięciu.
...
/// Prostokąty przycisków — wspólna geometria dla rysowania i hit-testów.
pub fn button_rects(r: Rect, window_h: f32) -> [Rect; 2] {
    let vh = window_h / 100.0;
    let btn_h = 4.2 * vh;
    let gap = 1.2 * vh;
    let top = r.y + 3.4 * vh; // miejsce na tytuł modułu
...
impl Control {
    ...
    /// Klik; zwraca indeks trafionego przycisku (BTN_EXIT / BTN_SETTINGS).
    ...
    pub fn draw(&mut self, ctx: &mut Ctx, r: Rect) {
        ...
        for (i, br) in rects.iter().enumerate() {
            ...
            // Równoległobok jak zakładki terminala (skewX).
            let skew = br.h * 0.7;
            ctx.dl.quad(
                [
                    [br.x + skew, br.y],

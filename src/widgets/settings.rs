//! Modalne okno ustawień (wyśrodkowane). Widok główny zawiera pozycję
//! THEMES; po jej kliknięciu u góry po lewej pojawia się przycisk powrotu,
//! a obok i poniżej lista motywów przeskanowanych z ~/.config/ng-term/themes.
...
pub struct Settings {
    ...
    /// Nazwa aktualnie ustawionego motywu (wyróżniona na liście).
    current: Option<String>,
    hits: Vec<(Rect, Act)>,
    flash: Option<(Act, Instant)>,
}

/// Prostokąt okna modalnego.
...
impl Settings {
    ...
    pub fn click(&mut self, x: f32, y: f32, w: f32, h: f32) -> Option<String> {
        ...
        if let Some(act) = act {
            ...
            match act {
                Act::OpenThemes => {
                    // Skan motywów w momencie otwierania widoku THEMES.
    ...
    pub fn draw(&mut self, ctx: &mut Ctx) {
        ...
        match self.view {
            ...
            View::Themes => {
                // Przycisk powrotu u góry po lewej.
                ...
                // Motywy: obok przycisku powrotu i niżej, wierszami.
                let cols = 3usize;
                let bw = (content.w - gap * (cols as f32 - 1.0)) / cols as f32;
                let mut col = 1usize; // pierwszy wiersz zaczyna się obok BACK
                let mut y = content.y;
    ...
    /// Przycisk w stylu zakładek terminala (skos, hover, błysk po kliknięciu).
    fn button(&mut self, ctx: &mut Ctx, r: Rect, label: &str, act: Act) {
        ...
        let flash = self
            ...
            .unwrap_or(false);
        // Aktualnie ustawiony motyw wyróżniamy jak aktywną zakładkę.
        ...
        // Strzałka w lewo przy przycisku powrotu.
        if act == Act::Back {

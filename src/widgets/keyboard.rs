//! Klawiatura ekranowa — replika klawiatury eDEX-UI (układ en-US).
//! Klikalna: wysyła sekwencje do PTY, z lepkimi modyfikatorami SHIFT/CTRL/ALT/FN.
...
pub enum Action {
    /// Zwykły znak; drugi element to wariant z SHIFT.
    Char(char, char),
    /// Stała sekwencja bajtów.
    Seq(&'static [u8]),
    Shift,
    Ctrl,
    Alt,
...
pub struct Keyboard {
    ...
    pub alt: bool,
    pub fn_mod: bool,
    /// Prostokąty klawiszy z ostatniej klatki: (rect, wiersz, kolumna).
    hits: Vec<(Rect, usize, usize)>,
    /// Czas ostatniego wciśnięcia (podświetlenie) per klawisz.
...
impl Keyboard {
    ...
    /// Podświetl klawisz odpowiadający znakowi z fizycznej klawiatury.
    ...
    /// Obsługa kliknięcia; zwraca bajty do wysłania do PTY.
    pub fn click(&mut self, x: f32, y: f32) -> Option<Vec<u8>> {
        ...
        match action {
            ...
            Action::Char(base, shifted) => {
                let mut out = Vec::new();
                // FN + cyfra = klawisz funkcyjny (jak w eDEX).
                if self.fn_mod {
    ...
    pub fn draw(&mut self, ctx: &mut Ctx, r: Rect) {
        ...
        for (ri, row) in self.rows.iter().enumerate() {
            ...
            for (ki, key) in row.iter().enumerate() {
                ...
                // Stan klawisza: wciśnięty niedawno albo aktywny lepki modyfikator.
                let flash = self
                    .pressed
                    .get(&(ri, ki))
                ...
                // Etykieta główna na środku. Strzałki rysujemy wektorowo,
                // bo font UI może nie mieć tych glifów.
                ...
                // Wariant SHIFT w prawym górnym rogu.

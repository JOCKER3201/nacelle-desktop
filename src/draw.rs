//! Lista rysowania — wszystko jako trójkąty (jeden pipeline, jeden atlas).
...
impl DrawList {
    ...
    /// Dowolny czworokąt (wierzchołki po obwodzie).
    ...
    /// Ramka ze ściętymi narożnikami w stylu augmented-ui (eDEX-owe panele).
    ...
    fn glyph_quad(&mut self, g: &Glyph, pen_x: f32, baseline: f32, color: Color) {
        ...
        let y1 = (baseline - g.ymin).round(); // dół bitmapy
        let y0 = y1 - g.h;
        let x1 = x0 + g.w;
        self.push_quad(
    ...
    /// Rysuje tekst; (x, y) to lewy górny róg pola tekstu. Zwraca szerokość.
    pub fn text(
        &mut self,
        fs: &mut FontSystem,
        font: u8,
    ...
    /// Tekst wyśrodkowany w poziomie względem cx.
    ...
    /// Tekst wyrównany do prawej krawędzi rx.
    #[allow(clippy::too_many_arguments)]
    pub fn text_right(
        &mut self,
        fs: &mut FontSystem,
    ...
    /// Nagłówek modułu w stylu eDEX: podkreślenie z "wąsami" + tekst
    /// po lewej i opcjonalnie po prawej.
    #[allow(clippy::too_many_arguments)]
    pub fn module_title(
        &mut self,
        fs: &mut FontSystem,
